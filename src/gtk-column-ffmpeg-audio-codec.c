/*
 * gtk-column-ffmpeg-audio-codec.c
 *
...
FM_DEFINE_MODULE(gtk_folder_col, ffmpeg_audio_codec)
...
void get_value( FmFileInfo *fi, GValue *value ){
	...
	g_value_set_static_string( value, acodec->name );
};
...
gint sort( FmFileInfo *fi1, FmFileInfo *fi2 ) {
	...
	if ( acodec1 == NULL && acodec2 == NULL ) {
		...
		return strcmp( acodec1->name, acodec2->name );
	}
}
