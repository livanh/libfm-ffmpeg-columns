/*
 * gtk-column-ffmpeg-video-codec-long.c
 *
...
FM_DEFINE_MODULE(gtk_folder_col, ffmpeg_video_codec_long)
...
void get_value( FmFileInfo *fi, GValue *value ){
	...
	g_value_set_static_string( value, vcodec->long_name );
};
...
gint sort( FmFileInfo *fi1, FmFileInfo *fi2 ) {
	...
	if ( vcodec1 == NULL && vcodec2 == NULL ) {
		...
		return strcmp( vcodec1->long_name, vcodec2->long_name );
	}
}
